...
//                        toTime: toTime,
            ...
            function renderLogListAndPage(result, url) {
                if (result.rect) {
                    if (result.data.totalCounts > 0) {
                        var rendered = Mustache.render(logListTemplate, {
                            ...
                            "showNewValue": function () {
                                return function (text, render) {
                                    return this.newValue ?this.newValue: '无';
                                }
                            }
                        });
                    ...
                    var type=$("#search-type").val();
                    var operator=$("#search-operator").val();
                    renderPage(url, result.data.totalCounts, pageNo, pageSize, result.data.totalCounts > 0 ? result.data.list.length : 0, "logPage", renderLogListAndPage,type,operator);
                } else {
