...
function show(totalCounts,from,to,currPage,pageSize,pageCount,departmentId) {
    document.getElementById("turnPage").innerHTML = '<div class="col-xs-6"> ' +
        ...
        '<a href="#" onclick=loadUsers(1,'+pageSize+','+departmentId+') class="page-action">首页</a> </li> ' +
        '<li class="paginate_button {{^beforeUrl}}disabled{{/beforeUrl}}" aria-controls="dynamic-table" tabindex="0"> ' +
        '<a href="#" onclick=loadUsers('+(currPage-1)+','+pageSize+','+departmentId+') class="page-action" id="previous_button">前一页</a> </li> ' +
        ...
        '<a href="#" onclick=loadUsers('+(currPage+1)+','+pageSize+','+departmentId+') class="page-action" id="paginate_button">后一页</a> </li> ' +
        '<li class="paginate_button next {{^lastUrl}}disabled{{/lastUrl}}" aria-controls="dynamic-table" tabindex="0"> ' +
        '<a href="#" onclick=loadUsers('+pageCount+','+pageSize+','+departmentId+') class="page-action">尾页</a> </li> ' +
        '</ul> </div> </div>';
}
